    ...
    private void showDeniedResponse(int[] grantResults) {
        if (grantResults.length > 1) {
            for (int i = 0; i < grantResults.length; i++) {
                if (grantResults[i] != 0) {
                    Toast.makeText(this, getString(R.string.permission_unavailable, permissionFeatures.values()[i]), Toast.LENGTH_SHORT).show();
                }
            }
        } else {
            Toast.makeText(this, getString(R.string.permission_unavailable, permissionFeatures.MICROPHONE), Toast.LENGTH_SHORT).show();
        }
    }
